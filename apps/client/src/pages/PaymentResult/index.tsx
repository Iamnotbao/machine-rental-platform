...
export function PaymentSuccessPage() {
  ...
  return <main className={styles.page}><section className={`${styles.card} ${styles.result} ${styles.receipt}`}><div className={`${styles.resultMark} ${styles.resultSuccess}`}>OK</div><p className={styles.eyebrow}>Payment successful</p><h1>Thanh toán thành công</h1><p>Đơn thuê đã được ghi nhận trong dữ liệu mock.</p><div className={styles.row}><span>Mã purchase</span><strong>{params.get('purchaseId') ?? 'PUR-DEMO'}</strong></div><div className={styles.row}><span>Provider</span><strong>{params.get('provider') ?? 'provider-a'}</strong></div><div className={styles.row}><span>Phương thức</span><strong>{params.get('method') ?? 'momo'}</strong></div><div className={styles.row}><span>Số tiền</span><strong>{money.format(Number(params.get('total')) || 0)}</strong></div><div className={styles.actions}><Link className={styles.primary} to={ROUTES.paymentHistory}>Xem lịch sử thanh toán</Link><Link className={styles.secondary} to={ROUTES.machines}>Thuê thêm máy</Link></div><div className={styles.receiptTeeth} aria-hidden="true"/></section></main>;
}
